...
class Scheduler:
    ...
    def wait(self):
        ...
        if lenght == 0:
            ...
            if all(waker.is_empty() for waker in self.wakers.values()):
                ...
                non_empty_wakers = filter(
                    lambda x: not x.is_empty(), self.wakers.values()
                )
                ...
                if sleep_duration > 0:  # this happens sometimes
                    ...
                    self.log(
                        "{:.6f}, {}, {:.6f}".format(
                            sleep_duration, type(waker).__name__, actual_sleep_duration
                        )
                    )
                else:
                    pass
    ...
    def run_until_completion(self):
        ...
        self.log(
            "total uptime (seconds): {:.6f}/{:.6f}".format(
                dilation - self.total_sleep, dilation
            )
        )
